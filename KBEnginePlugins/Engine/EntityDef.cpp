...
void EntityDef::initScriptModules()
{
	...
	TArray<DATATYPE_BASE*> Account_ESay_args;
	Account_ESay_args.Add(EntityDef::id2datatypes[12]);

	Method* pAccount_ESay = new Method();
	pAccount_ESay->name = TEXT("ESay");
	pAccount_ESay->methodUtype = 1;
	pAccount_ESay->aliasID = -1;
	pAccount_ESay->args = Account_ESay_args;

	pAccountModule->methods.Add(TEXT("ESay"), pAccount_ESay); 
	pAccountModule->cell_methods.Add(TEXT("ESay"), pAccount_ESay);

	pAccountModule->idcell_methods.Add(pAccount_ESay->methodUtype, pAccount_ESay);

	//DEBUG_MSG("EntityDef::initScriptModules: add(Account), method(ESay / 1).");
...
void EntityDef::initDefTypes()
{
	...
	{
		uint16 utype = 22;
		FString typeName = TEXT("CHAI_INFO");
		DATATYPE_CHAI_INFO* pDatatype = new DATATYPE_CHAI_INFO();
