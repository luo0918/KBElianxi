...
void ClientSDKUpdater::onImportClientSDK(const UKBEventData* eventData)
{
	...
	if (pSdkFileStream->length() == pEventData->fileSize)
	{
		...
		if (pEventData->remainingFiles == 0)
		{
			...
			UKBEventData_onImportClientSDKSuccessfully* pEventDatal = NewObject<UKBEventData_onImportClientSDKSuccessfully>();

			KBENGINE_EVENT_FIRE(KBEventTypes::onImportClientSDKSuccessfully, pEventDatal);
		}
	}
}
